...
const startIdx = content.indexOf('{/* 2. Service Cards list */}');
const endIdx = content.lastIndexOf('</div>\n    </div>\n  );\n}');

const serviceSectionItem = `
function ServiceSectionItem({ service, t, language, handleContactService }: any) {
...
const replaceExportDefault = content.indexOf('export default function ServicesPage');

let newContent = content.substring(0, replaceExportDefault) + serviceSectionItem + content.substring(replaceExportDefault + 'export default function ServicesPage'.length);

const startIdx2 = newContent.indexOf('{/* 2. Service Cards list */}');
const endIdx2 = newContent.lastIndexOf('</div>\n    </div>\n  );\n}');

const newSection = `      {/* 2. Service Cards list */}
...
newContent = newContent.substring(0, startIdx2) + newSection + newContent.substring(endIdx2);

fs.writeFileSync('src/components/ServicesPage.tsx', newContent);
